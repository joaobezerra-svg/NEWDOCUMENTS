...
# Safe imports to prevent startup crashes
try:
...
def get_services():
    ...
    if not creds:
        raise Exception("Credenciais não encontradas! Configure GOOGLE_CREDENTIALS no Vercel ou crie credentials.json localmente.")
...
def processar():
    try:
        ...
        # --- ARMAZENAMENTO ---
        # Define caminhos ABSOLUTOS para evitar erros
        ...
        if formato == 'PDF':
            try:
                if os.name == 'nt':
                    ...
                    return send_file(temp_pdf_path, as_attachment=True, download_name="Portaria_Final.pdf", mimetype="application/pdf")
                else:
                    return {"error": "A geração de PDF requer Windows/Word. No Vercel, use 'GERAR NO DOCS'."}, 400
            except Exception as e:
